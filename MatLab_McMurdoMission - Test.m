...
switch propagatormode
    case 1
        ...
        GHawk.SetRouteType('ePropagatorGreatArc'); %mess with this and Aviator
        ...
        GHawkRoute.Method = 'eDetermineTimeAccFromVel';
        GHawkRoute.SetAltitudeRefType('eWayPtAltRefTerrain');
    case 2
        GHawk = scenario.Children.New('eAircraft', 'GlobalHawk');
        ...
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Procedure Add After 1 SiteType Waypoint ProcedureType "Basic Point to Point"'])
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Site 2 SetValue Latitude -79.800 deg']);
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Site 2 SetValue Longitude 168.0000 deg']);
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Procedure Add AfterLast SiteType Runway ProcedureType "Landing"']); 
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Site 3 SetValue Latitude -80.800 deg']);
        %root.ExecuteCommand(['MissionModeler */Aircraft/GlobalHawk Site 3 SetValue Longitude 168.0000 deg']);
...
while (notclose)
    round = round + 1;
    GHawkWaypointGeneratorGA(GHawkRoute, defaultGHawkWPs, round);
...
while (done == false)
    round2 = round2 + 1;
    %disp(round2);
    GHawkWaypointGeneratorGA(GHawkRoute, defaultGHawkWPs, round2);
    if round2 == (round - 1) %optimize this part in the future
        waypoint = GHawkRoute.Waypoints.Add();
        waypoint.Latitude = GHawk_Partial_LLA(matLength,2);
        waypoint.Longitude = GHawk_Partial_LLA(matLength,3);
...
waypoint = GHawkRoute.Waypoints.Add();
waypoint.Latitude = realTarget_lat;
waypoint.Longitude = realTarget_long;
waypoint.Altitude = 8; %km
waypoint.Speed = 0.05; %km/sec\
waypoint.TurnRadius = 0; %km
GHawkRoute.Propagate;
...
for i = 1:10
     GHawkWaypointGeneratorGA(GHawkRoute, defaultGHawkWPs, 5);
     GHawkWaypointGeneratorGA(GHawkRoute, defaultGHawkWPs, 4);
     waypoint = GHawkRoute.Waypoints.Add();
    ...
    CircularCmd = ['MissionModeler */Aircraft/GlobalHawk Procedure ' num2str(i) ' SetValue Turns 5'];
    %root.ExecuteCommand(CircularCmd)
